...
#'
#' @param gene_rank_statistics \code{vector} containing gene rank statistics
#' calculated by \code{\link{calculate_rank_statistic}}
...
#' @param pvalue_col \code{character} giving name of column containing gene
#' p-values.
#' @param randomize_gene_labels \code{boolean}. If \code{TRUE}, randomize gene
#' labels in \code{gene_pvalues} to perform permutations.
...
calculate_all_truncation_rtp <- function(rank_statistics) {
  ...
  # Calcualate RTP for each truncation point for (i in
  # 1:nrow(gene_set_pvalues$pvalues)){ test_rtp <- calculate_rtp(
  # gene_rank_statistics = gene_set_pvalues$pvalues$rank_statistic, num_genes = i)
  # all_rtp[[i]] <- test_rtp names(all_rtp[[i]]) <- paste('truncation_point_', i,
  # sep = '') } all_rtp <- unlist(all_rtp) gene_set_pvalues$all_rtp <- all_rtp
  return(all_rtp)
}
...
calculate_artp_perm <- function(gene_set, gene_pvalues, n_perm = 1000) {
  ...
  for (i in 1:n_perm) {
    ...
    simulated_rtp_results[[i]] <- calculate_all_truncation_rtp(
      simulated_rank_statistic
    )
  }
  ...
  artp_pvalue <- count_pvalues_less_than_observed /
    (length(min_truncation_pvalues))
